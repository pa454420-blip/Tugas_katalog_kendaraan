...
function KartuKendaraan({ nama, jenis, harga = 0, status, gambar }) {
  return (
    <div className="rounded-lg border p-4 mb-3 bg-blue-100 shadow-sm flex flex-col justify-between">
      <div>
        {gambar && (
          <img
            src={gambar}
        ...
        <p className="text-sm font-medium mt-1">
          Harga: <span className="font-bold text-gray-800">Rp{harga.toLocaleString("id-ID")}</span> / hari
        </p>
        {status && (
          <p
      ...
      {/* Hitung Sewa Per Hari */}
      {status && status.toUpperCase() === "TERSEDIA" ? (
        <Calculator harga={harga} />
      ) : (
        <div className="mt-3 p-2 bg-gray-200/60 rounded text-center text-xs text-gray-600">
          Mobil sedang tidak tersedia untuk disewa
        </div>
      )}
    </div>
  );
...
export default function BerandaPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4">
      <h1 className="text-4xl text-black-600 font-bold mb-6 text-center">
